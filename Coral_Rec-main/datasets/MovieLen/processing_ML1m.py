import csv
import pickle
import os
from collections import defaultdict

def process_and_partition_movielens(csv_file, output_dir="processed_data"):
    print(f"--- Start processing file: {csv_file} ---")
    
    # 1. Initialize ID mapping dictionaries
    user2id = {} 
    item2id = {} 
    cat2id  = {} 
    
    # Storage structure: UserID -> List of (Timestamp, ItemID, CatID, Rating)
    # Modification: Rating is added to the tuple
    User_interactions = defaultdict(list)
    
    # 2. Read CSV directly
    try:
        with open(csv_file, 'r', encoding='utf-8') as f:
            reader = csv.reader(f)
            header = next(reader)  # Skip header
            print(f"Detected header: {header}")
            
            count = 0
            for row in reader:
                # Column order saved by Pandas in the previous step:
                # final_df = ratings_df[['user_id', 'item_id', 'rating', 'timestamp', 'category']]
                # row[0]=user_id, row[1]=item_id, row[2]=rating, row[3]=timestamp, row[4]=category
                
                u_origin = row[0]
                i_origin = row[1]
                r_origin = float(row[2])  # New: read rating (cast to float)
                t_origin = row[3]
                c_origin = row[4]
                
                # --- ID mapping logic ---
                if u_origin not in user2id:
                    user2id[u_origin] = len(user2id) + 1
                if i_origin not in item2id:
                    item2id[i_origin] = len(item2id) + 1
                if c_origin not in cat2id:
                    cat2id[c_origin] = len(cat2id) + 1
                
                u_int = user2id[u_origin]
                i_int = item2id[i_origin]
                c_int = cat2id[c_origin]
                
                # Store tuple: (time, item, category, rating)
                # Note: time is placed first for convenient sorting later
                User_interactions[u_int].append((t_origin, i_int, c_int, r_origin))
                
                count += 1
                if count % 1000000 == 0:
                    print(f"Processed {count} rows...")
                    
        print(f"Reading completed. Total interactions: {count}")
        print(f"Users: {len(user2id)}, Items: {len(item2id)}, Categories: {len(cat2id)}")

    except FileNotFoundError:
        print(f"Error: file not found {csv_file}")
        return

    # 3. Sort by timestamp and split (Train / Valid / Test)
    print("Sorting by timestamp and splitting datasets...")
    
    # Temporary storage for split results
    data_store = {
        'train': {},
        'valid': {},
        'test': {}
    }
    
    for u_int, interactions in User_interactions.items():
        # Sort by timestamp (x[0])
        interactions.sort(key=lambda x: x[0])
        
        # Unpack data: now contains 4 elements
        seq_times, seq_items, seq_cats, seq_ratings = zip(*interactions)
        
        seq_times   = list(seq_times)
        seq_items   = list(seq_items)
        seq_cats    = list(seq_cats)
        seq_ratings = list(seq_ratings)  # New
        
        n = len(seq_items)
        
        # --- Splitting logic ---
        if n < 3:
            # Too few interactions: put all into training set (Valid/Test empty)
            data_store['train'][u_int] = (seq_items, seq_cats, seq_times, seq_ratings)
            data_store['valid'][u_int] = ([], [], [], [])
            data_store['test'][u_int]  = ([], [], [], [])
        else:
            # Train: remove last two interactions
            data_store['train'][u_int] = (
                seq_items[:-2],
                seq_cats[:-2],
                seq_times[:-2],
                seq_ratings[:-2]
            )
            
            # Valid: second-to-last interaction as target
            data_store['valid'][u_int] = (
                seq_items[-2],
                seq_cats[-2],
                seq_times[-2],
                seq_ratings[-2]
            )
            
            # Test: last interaction as target
            data_store['test'][u_int] = (
                seq_items[-1],
                seq_cats[-1],
                seq_times[-1],
                seq_ratings[-1]
            )

    # 4. Generate Pickle files
    dump_to_pickle(data_store, output_dir)
    
    # Save ID mappings (optional)
    save_maps(user2id, item2id, cat2id, output_dir)


def dump_to_pickle(data_store, output_dir):
    # Updated description: Ratings are included
    print("Generating Pickle files (structure: [Inputs, User, Cats, Targets, Times, Ratings])...")
    os.makedirs(output_dir, exist_ok=True)

    # Initialize 6-list data structure
    # 0: Item sequence, 1: UserID, 2: Category sequence,
    # 3: Target item, 4: Time sequence, 5: Rating sequence
    datasets = {
        'train': [[], [], [], [], [], []],
        'valid': [[], [], [], [], [], []],
        'test':  [[], [], [], [], [], []]
    }

    # Ensure consistent user ordering
    users = sorted(data_store['train'].keys())

    for u in users:
        # Unpack Train (sequence)
        train_i, train_c, train_t, train_r = data_store['train'][u]
        # Unpack Valid (single item)
        valid_i, valid_c, valid_t, valid_r = data_store['valid'][u]
        # Unpack Test (single item)
        test_i,  test_c,  test_t,  test_r  = data_store['test'][u]
        
        # Check if valid/test data exists
        has_valid = valid_i != [] and isinstance(valid_i, int)
        has_test  = test_i  != [] and isinstance(test_i, int)

        # --- Fill Train ---
        datasets['train'][0].append(train_i)
        datasets['train'][1].append([u])
        datasets['train'][2].append(train_c)
        datasets['train'][3].append(valid_i if has_valid else 0)
        datasets['train'][4].append(train_t)
        datasets['train'][5].append(train_r)  # New: rating sequence for training

        # --- Fill Valid ---
        if has_valid:
            datasets['valid'][0].append(train_i)   # Input history
            datasets['valid'][1].append([u])
            datasets['valid'][2].append(train_c)   # Category history
            datasets['valid'][3].append(valid_i)   # Target item
            datasets['valid'][4].append(train_t)
            datasets['valid'][5].append(train_r)   # New: input rating sequence

        # --- Fill Test ---
        if has_test:
            # Test input usually includes the valid item (append mode)
            if has_valid:
                test_seq_i = train_i + [valid_i]
                test_seq_c = train_c + [valid_c]
                test_seq_t = train_t + [valid_t]
                test_seq_r = train_r + [valid_r]  # New: append rating
            else:
                test_seq_i = train_i
                test_seq_c = train_c
                test_seq_t = train_t
                test_seq_r = train_r
            
            datasets['test'][0].append(test_seq_i)
            datasets['test'][1].append([u])
            datasets['test'][2].append(test_seq_c)
            datasets['test'][3].append(test_i)     # Target item
            datasets['test'][4].append(test_seq_t)
            datasets['test'][5].append(test_seq_r) # New: test input rating sequence

    # Write to disk
    for mode, data_list in datasets.items():
        # Using .txt extension (pickle content), consistent with previous code
        path = os.path.join(output_dir, f"{mode}.txt")
        with open(path, 'wb') as f:
            pickle.dump(data_list, f)
        
        # Print statistics
        print(f"Saved {mode}.txt | Number of samples: {len(data_list[0])}")

def save_maps(user2id, item2id, cat2id, output_dir):
    with open(os.path.join(output_dir, 'id_map.pickle'), 'wb') as f:
        pickle.dump(
            {'user2id': user2id, 'item2id': item2id, 'cat2id': cat2id},
            f
        )
    print("ID mapping tables have been saved.")

if __name__ == "__main__":
    # Input file name (must be the file generated in the previous step with ratings)
    input_file = 'movielens_sequential_dataset.csv'
    process_and_partition_movielens(input_file)

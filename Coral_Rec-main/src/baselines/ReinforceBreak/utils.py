...
class Data(Dataset):
    def __init__(self, raw_data, num_items, max_len, mask_prob, mode='train'):
        self.raw_cats = [x[:] for x in raw_data[2]]
        self.raw_targets = raw_data[3][:]
        self.raw_ratings = [x[:] for x in raw_data[5]]
    ...
    def inject_exposure_data(self, new_edges):
        print(f"[Data] Injecting {len(new_edges)} controllable exposure edges to break filter bubbles...")
        
        cnt = 0
        for uid, item_id in new_edges:
            if uid < len(self.raw_inputs):
                # Append to sequence (Simulating user clicked the exposed item)
                self.raw_inputs[uid].append(item_id)
                
                # Handle category/rating alignment
                # Assuming positive implicit feedback (1.0) for the exposed item
                last_cat = self.raw_cats[uid][-1] if len(self.raw_cats[uid]) > 0 else 0
                self.raw_cats[uid].append(last_cat) 
                self.raw_ratings[uid].append(1.0) 
                cnt += 1
        
        print(f"[Data] Successfully updated {cnt} sequences. Refreshing dataset tensors...")
        self._refresh_data()
    ...
    def _prepare_data(self):
        ...
        for i in range(len(self.raw_inputs)):
            ...
            # Filter out 0-padding from raw data
            seq_clean = []
            cat_clean = []
            rate_clean = []
            ...
            if self.mode == 'train':
                # Training Mode: Input = seq[:-1], Target = seq[1:]
